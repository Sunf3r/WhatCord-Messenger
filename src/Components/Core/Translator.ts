...
class TranslateAPI {
    ...
    public async translate(content: string, to = 'en', from = 'auto') {
        ...
        body[0].forEach((obj: any) => obj[0] ? result.text += obj[0] : null);
    ...
    getLanguage(lang: string) {
        if (this.languages[String(lang).toLowerCase()])
            return this.languages[String(lang).toLowerCase()];
    }
}
